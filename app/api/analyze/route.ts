    ...
    data: [
      {
        ...
        value: confidence,
        title: { text: `<b>Confidence</b><br><span style="font-size:12px;color:#cbd5e1">Threat: ${predictedThreat}</span>` },
        number: { suffix: "%" },
        gauge: {
          axis: { range: [0, 100], tickwidth: 1, tickcolor: "#94a3b8" },
          bar: { color: "#0ea5a6" },
          steps: gaugeColors.map(([s, e, color]) => ({ range: [s * 100, e * 100], color })),
          threshold: { line: { color: "#111827", width: 4 }, thickness: 0.8, value: confidence },
        },
      },
    ],
    layout: {
...
async function buildTimelineFromDB() {
  ...
  const records = await db.collection(COLLECTION).find().sort({ firstSeen: 1 }).toArray();

  if (records.length === 0) return { data: [], layout: {} };
  ...
  return {
    ...
    layout: {
      title: { text: "All Reports Over Time", font: { size: 18 } },
      xaxis: { title: "First Seen", showgrid: true, gridcolor: "#1f2937", tickangle: -25, tickfont: { color: "#cbd5e1" } },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user_input) return NextResponse.json({ error: "Input is required" }, { status: 400 });
    ...
    const [report] = result.data as [string, string | null];
