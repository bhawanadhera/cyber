...
"[project]/node_modules/@gradio/client/dist/index.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function resolve_cookies() {
    ...
    try {
        if (this.options.auth) {
            const cookie_header = await get_cookie_header(http_protocol, host, this.options.auth, this.fetch, this.options.token);
...
function transform_api_info(api_info, config, api_map) {
    ...
    Object.keys(api_info).forEach((category)=>{
        if (category === "named_endpoints" || category === "unnamed_endpoints") {
            ...
            Object.entries(api_info[category]).forEach(([endpoint, { parameters, returns }])=>{
                var _a, _b, _c, _d;
...
async function handle_blob(endpoint, data, api_info) {
    ...
    const results = await Promise.all(blobRefs.map(async ({ path, blob, type })=>{
        if (!blob) return {
            path,
            type
        };
        const response = await self.upload_files(endpoint, [
            blob
        ]);
...
async function recursively_process_commands(client2, data, path = []) {
    for(const key in data){
        if (data[key] instanceof Command) {
            ...
            await recursively_process_commands(client2, data[key], [
                ...path,
                key
            ]);
        }
...
async function process_single_command(client2, data, key) {
    ...
    try {
        ...
        const response = await client2.upload_files(root, [
            file
        ]);
        const file_url = response.files && response.files[0];
        if (file_url) {
...
function submit(endpoint, data = {}, event_data, trigger_id, all_events) {
    ...
    try {
        ...
        const { token } = this.options;
        const { fetch: fetch2, app_reference, config, session_hash, api_info, api_map, stream_status, pending_stream_messages, pending_diff_streams, event_callbacks, unclosed_events, post_data: post_data2, options, api_prefix } = this;
        const that = this;
        ...
        let complete = false;
        let last_status = {};
        let url_params = ("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : "";
        ...
        this.handle_blob(config.root, resolved_data, endpoint_info).then(async (_payload)=>{
            ...
            if (skip_queue(fn_index, config)) {
                ...
                post_data2(`${config.root}${api_prefix}/run${_endpoint.startsWith("/") ? _endpoint : `/${_endpoint}`}${("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : ""}`, {
                    ...payload,
                    session_hash
                ...
                let url = new URL(`${config.root}${api_prefix}/${SSE_URL}?${("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : ""}${params}`);
                if (this.jwt) {
                ...
                const is_zerogpu_iframe = ("TURBOPACK compile-time value", "undefined") !== "undefined" && typeof document !== "undefined" && window.parent != window && window.supports_zerogpu_headers;
                const zerogpu_auth_promise = ("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : Promise.resolve(null);
                ...
                post_data_promise.then(async ([response, status])=>{
                    if (status === 503) {
                        ...
                        let callback = async function(_data) {
                            try {
                                const { type, status: status2, data: data2, original_msg } = handle_message(_data, last_status[fn_index]);
                                if (type == "heartbeat") {
                                    return;
                                }
                                if (type === "update" && status2 && !complete) {
                                    ...
                                    if (data2 && dependency.connection !== "stream" && [
                                        "sse_v2",
                                        "sse_v2.1",
                                        "sse_v3"
                                    ].includes(protocol)) {
...
class Client {
    ...
    async init() {
        ...
        await this._resolve_config().then(({ config })=>this._resolve_heartbeat(config));
        ...
        if (_config) {
            ...
            if (this.config && this.config.connect_heartbeat) {
                if (this.config.space_id && this.options.token) {
                    this.jwt = await get_jwt(this.config.space_id, this.options.token, this.cookies);
                }
            }
        }
...
//# sourceMappingURL=node_modules_4187df33._.js.map